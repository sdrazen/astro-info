...
import { interval, Observable, Subscription, map } from 'rxjs';
import { IIssModel } from './shared/data.iss.model';
@Component({
    selector: 'app-iss',
...
export class IssComponent implements OnInit {
    ...
    currentIssPosition$: Observable<IIssModel>;
    ...
    constructor(private _userSettingsService: UserSettingsService, private _translationsService: TranslationsService, private _issService: IssService) { }
    ...
    ngOnInit() {
        ...
        this.intervalObservableSubscription$ = interval(this.refreshPeriod).pipe(
            map(() => {
                this._issService.getCurrentIssPosition().pipe(
                    map(data => {
                        this.lat = data.latitude;
                        this.lng = data.longitude;
                        this.altitude = Math.round(data.altitude);
