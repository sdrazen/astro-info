...
import { IDataStoreMarkerModel } from './shared/data.storemarker.model';
...
export class StoresComponent implements OnInit {
  ...
  ngOnInit() {
      ...
      //   this.stores = [];
      //   this._firebaseDataService.getAllStores("storecountry")
      //     .then(() => {
      //           this.stores = this._firebaseDataService.allStores;
      //       });
  ...
  onDeleteClick(storeKey) {
      ...
      this._firebaseDataService.deleteStore(storeKey)
        .then(() => {
            this._firebaseDataService.allStores = [];
            this.stores = [];
            this._firebaseDataService.getAllStores("storecountry")
                .then(() => {
                        this.stores = this._firebaseDataService.allStores;
  ...
  onSaveNewClick() {
      ...
      this._firebaseDataService.addStore(newStore)
  ...
  onUpdateExistingClick(storeKey: string) {
      ...
      this._firebaseDataService.updateStore(storeKey, updatedStore)
        .then((store) => {
  ...
  onMapClick($event: any) {
      ...
      if (this.mode === dataHandlingMode.add) {
        ...
        this.newStoreStoreLat = $event.coords.lat;
        this.newStoreStoreLng = $event.coords.lng;
      } 
      else if (this.mode === dataHandlingMode.update) {
        // Mode is "update", update coordinates
        this.updatedStoreStoreLat = $event.coords.lat;
        this.updatedStoreStoreLng = $event.coords.lng;
      } else {
