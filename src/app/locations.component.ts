...
export class LocationsComponent implements OnInit {
  ...
  onDeleteClick(locationKey) {
    ...
    this._backendService.deleteLocation(locationKey)
      .then(() => {
        this._backendService.allLocations = [];
        this.locations = [];
        this._backendService.getAllLocations("locationcountry")
          .then(() => {
            this.locations = this._backendService.allLocations;
  ...
  onSaveNewClick() {
    ...
    this._backendService.addLocation(newLocation)
  ...
  onUpdateExistingClick(locationKey: string) {
    ...
    this._backendService.updateLocation(locationKey, updatedLocation)
      .then((location) => {
  ...
  onMapClick($event: google.maps.MapMouseEvent) {
    ...
    if (this.mode === dataHandlingMode.add) {
      ...
      this.newLocationLocationLat = $event.latLng.lat();
      this.newLocationLocationLng = $event.latLng.lng();
    }
    else if (this.mode === dataHandlingMode.update) {
      // Mode is "update", update coordinates
      this.updatedLocationLocationLat = $event.latLng.lat();
      this.updatedLocationLocationLng = $event.latLng.lng();
    } else {
